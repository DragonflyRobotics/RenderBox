...
int main() {
  ...
  printf("Loaded model with %lu triangles\n", obj.triangles.size());
  mesh_model(obj);
  obj.rotateY(M_PI - 0.1);
  obj.rotateZ(M_PI);
  mesh2screen(obj);
  while (true) {
    XSetForeground(handle.display, handle.graphics_context,
                   RGB2Hex(255, 255, 255));
    XFillRectangle(handle.display, handle.backbuffer, handle.graphics_context,
                   0, 0, 640, 480);
    // obj.rotateX(M_PI/10);
    ...
    std::vector<std::vector<float>> zbuffer(
        480, std::vector<float>(640, std::numeric_limits<float>::infinity()));
    for (const Triangle3d &t : obj.triangles) {
      Triangle2d small_t = Triangle2d{Point2d{(int)t.a.x, (int)t.a.y},
        ...
        for (int h = miny; h <= maxy; h++) {
          if (small_t.is_point_inside(Point2d{w, h})) {
            ...
            if (d < zbuffer[h][w]) {
              ...
              continue; // Skip if the depth is not less than the current
                        // zbuffer value
            }
            int red_intensity =
                max_brightness - (int)(((d - obj.bbox[0][2]) /
                                        (obj.bbox[1][2] - obj.bbox[0][2])) *
                                       max_brightness);
